-- Add article_source_url to get_public_topic_feed function
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(s.title, sac.title, a.title) as title,
    COALESCE(a.summary, sac.body) as summary,
    COALESCE(s.author, sac.author, a.author) as author,
    s.status,
    s.is_published,
    s.created_at,
    s.updated_at,
  ...
  GROUP BY s.id, s.article_id, sac.url, a.source_url, s.title, sac.title, a.title, 
           a.summary, sac.body, s.author, sac.author, a.author, s.status, s.is_published, 
