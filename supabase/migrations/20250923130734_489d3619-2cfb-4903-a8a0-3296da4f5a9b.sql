-- Add date filtering to get_topic_stories RPC function to only return last 30 days
...
BEGIN
  ...
  -- Return stories from both legacy and multi-tenant sources (last 30 days only)
...
-- Add composite index for stories performance
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stories_created_at_status_published 
ON stories (created_at DESC, status, is_published) 
WHERE is_published = true AND status IN ('ready', 'published');

-- Add comment for documentation
COMMENT ON FUNCTION public.get_topic_stories IS 'Retrieves paginated stories for a topic, filtered to last 30 days for performance';