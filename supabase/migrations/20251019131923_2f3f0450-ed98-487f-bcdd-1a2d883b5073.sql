-- Rollback: Restore get_topic_stories_with_keywords to accept text[] domains instead of uuid[]
-- This fixes the 404 error caused by signature mismatch with the frontend

-- First, explicitly drop all versions of the function
DROP FUNCTION IF EXISTS public.get_topic_stories_with_keywords(text, text[], text[], integer, integer);
DROP FUNCTION IF EXISTS public.get_topic_stories_with_keywords(text, text[], uuid[], integer, integer);
DROP FUNCTION IF EXISTS public.get_topic_stories_with_keywords(uuid, text[], integer, integer);
DROP FUNCTION IF EXISTS public.get_topic_stories_with_keywords(text, text[], integer, integer);

-- Recreate with text[] for p_sources to match frontend expectations
CREATE OR REPLACE FUNCTION public.get_topic_stories_with_keywords(
  p_topic_slug text,
  p_keywords text[] DEFAULT NULL,
  p_sources text[] DEFAULT NULL,  -- Back to domain text[] to match frontend
