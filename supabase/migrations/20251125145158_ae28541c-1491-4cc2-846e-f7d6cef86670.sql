-- Fix type mismatch in get_topic_sources RPC function
-- credibility_score is integer in the table, not numeric
...
RETURNS TABLE (
  ...
  is_active boolean,
  source_type text,
  credibility_score integer,  -- ✅ Fixed: was numeric, should be integer
...
BEGIN
  ...
  SELECT 
    ts.source_id,
    csb.source_name,
    -- Use content_sources.feed_url instead of falling back to domain
    COALESCE(
