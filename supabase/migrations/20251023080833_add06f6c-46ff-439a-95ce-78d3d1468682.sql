-- Safe MP filter fix: Prevent story duplication and enforce parliamentary-only filtering
-- This migration drops and recreates get_topic_stories_with_keywords with proper MP aggregation

DROP FUNCTION IF EXISTS public.get_topic_stories_with_keywords(uuid, text[], text[], text[], int, int);
...
BEGIN
  ...
  WITH base_stories AS (
    ...
    FROM stories s
    INNER JOIN topic_stories ts ON ts.story_id = s.id
    ...
    WHERE ts.topic_id = p_topic_id
      ...
      AND (p_keywords IS NULL OR EXISTS (
        SELECT 1 FROM topic_story_keywords tsk
        WHERE tsk.story_id = s.id 
          AND tsk.keyword = ANY(p_keywords)
      ))
      AND (p_source_domains IS NULL OR cs.domain = ANY(p_source_domains))
      -- CRITICAL: When MP filter is active, only show parliamentary stories
  ...
  mp_aggregates AS (
    SELECT 
      pm.story_id,
      -- Aggregate all normalized MP names for array overlap filtering
      ...
      -- Take first MP mention for display (maintains existing return shape)
  ...
  WHERE sl.id IS NOT NULL
    -- Apply MP filter using array overlap (server-side filtering)
