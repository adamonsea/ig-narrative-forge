-- Fix: Only show published stories in public feed and story pages
-- Previously allowed 'ready' status which exposed scheduled stories

-- Fix get_public_story_by_slug_and_id to only show published stories
...
BEGIN
  ...
  WHERE s.id = p_story_id
    ...
    AND s.status = 'published'  -- FIXED: Only published, not ready
