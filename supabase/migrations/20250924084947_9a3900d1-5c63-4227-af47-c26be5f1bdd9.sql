-- Clean up conflicting RPC functions and create one unified function for admin pipeline
-- First drop the conflicting functions
DROP FUNCTION IF EXISTS public.get_topic_stories(p_topic_slug text, p_limit integer, p_offset integer, p_sort_by text);
DROP FUNCTION IF EXISTS public.get_topic_stories(p_topic_id uuid, p_status text, p_limit integer, p_offset integer);
DROP FUNCTION IF EXISTS public.get_unified_topic_stories(p_topic_id uuid, p_status text, p_limit integer, p_offset integer);

-- Create a single, clean admin pipeline function for getting topic stories
