-- Fix ambiguous column reference error in get_topic_stories_with_keywords
-- Wrap UNION query in outer SELECT to allow proper ORDER BY
...
BEGIN
  ...
  -- Wrap the UNION in an outer query for unambiguous ORDER BY
  RETURN QUERY
  SELECT * FROM (
  ...
  -- Now ORDER BY works without ambiguity
  ORDER BY COALESCE(article_published_at, story_created_at) DESC, slide_number ASC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$;
