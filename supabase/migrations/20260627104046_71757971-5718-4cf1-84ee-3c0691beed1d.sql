    ...
    CASE WHEN v_has_keyword_filters THEN
      ARRAY(
        ...
        WHERE LOWER(s.title) LIKE '%' || LOWER(kf) || '%'
           OR LOWER(sac.title) LIKE '%' || LOWER(kf) || '%'
           OR EXISTS (
             SELECT 1 FROM slides sl2
             WHERE sl2.story_id = s.id
             AND LOWER(sl2.content) LIKE '%' || LOWER(kf) || '%'
           )
      )
...
GRANT EXECUTE ON FUNCTION public.get_public_topic_feed(text, integer, integer, text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_topic_stories_with_keywords(uuid, text[], text[], integer, integer) TO anon, authenticated;