-- Update the popular stories function to use 'this_month' instead of 'older' and limit to 1 month
...
BEGIN
  ...
  WITH story_swipes AS (
    SELECT 
      ...
      CASE 
        ...
        WHEN si.created_at >= CURRENT_DATE - INTERVAL '30 days' THEN 'this_month'
        ELSE 'archived'
      END as period_type
  ...
  ranked_stories AS (
    ...
    WHERE ss.period_type != 'archived' -- Exclude archived stories from popular ranking
  )
  SELECT 
    rs.story_id,
    rs.period_type,
  ...
  WHERE 
    ...
    (rs.period_type = 'this_month' AND rs.rank_position <= 2);
END;
$$;