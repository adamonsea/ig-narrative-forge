-- Create consolidated dashboard stats RPC that returns all stats for a user's topics in ONE call
...
BEGIN
  ...
  -- Arrivals count (new/processed articles not yet published or queued)
  arrivals AS (
    SELECT 
      ta.topic_id,
      COUNT(*) FILTER (
        WHERE ta.processing_status IN ('new', 'processed')
        AND NOT EXISTS (
  ...
  engagement_avgs AS (
    ...
    FROM (
      SELECT 
        ...
        0 as final_cnt -- simplified for performance
      FROM story_interactions si
      WHERE si.topic_id IN (SELECT id FROM user_topics)
