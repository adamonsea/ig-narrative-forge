-- Update the get_public_story_by_slug_and_id function to include shared_content_id for canonical URL resolution
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'shared_content_id', ta.shared_content_id,  -- Added for canonical URL resolution
    'slides', (
      SELECT jsonb_agg(
        jsonb_build_object(
          'id', sl.id,
  ...
  -- Fall back to legacy architecture (no shared_content_id in this path)
  SELECT jsonb_build_object(
    'id', s.id,
    'title', s.title,
    'author', s.author,
    ...
    'created_at', s.created_at,
    'updated_at', s.updated_at,
    'shared_content_id', NULL,  -- Legacy stories don't have shared content
    'slides', (
      SELECT jsonb_agg(
        jsonb_build_object(
          'id', sl.id,
