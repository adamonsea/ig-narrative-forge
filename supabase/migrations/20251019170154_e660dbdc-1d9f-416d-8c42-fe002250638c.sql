-- Disambiguate ORDER BY by qualifying with subquery alias
...
BEGIN
  ...
  -- Wrap the UNION in an outer query and fully qualify ORDER BY columns
  RETURN QUERY
  SELECT * FROM (
  ...
  ORDER BY 
    COALESCE(combined_stories.article_published_at, combined_stories.story_created_at) DESC,
    combined_stories.slide_number ASC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$;
