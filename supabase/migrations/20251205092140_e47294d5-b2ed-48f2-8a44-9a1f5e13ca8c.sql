-- Drop and recreate RPC functions with correct columns (slides table doesn't have type, hook, image_url, cta_text, cta_url)
...
-- Recreate get_topic_stories_with_keywords with only existing slide columns
CREATE OR REPLACE FUNCTION public.get_topic_stories_with_keywords(p_topic_id uuid, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0, p_keyword_filters text[] DEFAULT NULL::text[], p_source_filters text[] DEFAULT NULL::text[])
...
-- Recreate get_public_topic_feed with only existing slide columns
CREATE OR REPLACE FUNCTION public.get_public_topic_feed(p_topic_slug text, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0)
