...
DECLARE
  ...
  v_prev_week_start date := CURRENT_DATE - 14;
...
BEGIN
  ...
  visitor_stats AS (
    SELECT 
      ...
      COUNT(DISTINCT sv.visitor_id) FILTER (WHERE sv.visit_date >= v_week_start AND sv.page_type IN ('feed', 'story')) as week,
      COUNT(DISTINCT sv.visitor_id) FILTER (WHERE sv.visit_date >= v_prev_week_start AND sv.visit_date < v_week_start AND sv.page_type IN ('feed', 'story')) as prev_week,
  ...
  subscriber_stats AS (
    SELECT 
      tns.topic_id,
      COUNT(DISTINCT tns.email) FILTER (WHERE tns.is_active = true AND tns.email IS NOT NULL AND tns.created_at >= v_week_start) as week,
      COUNT(DISTINCT tns.email) FILTER (WHERE tns.is_active = true AND tns.email IS NOT NULL) as total
    FROM topic_newsletter_signups tns
    WHERE tns.topic_id IN (SELECT id FROM user_topics)
    GROUP BY tns.topic_id
  ),
  engagement_avgs AS (
    SELECT 
      si.topic_id,
  ...
  SELECT 
    ut.id as topic_id,
    COALESCE(a.cnt, 0) as articles_in_arrivals,
    COALESCE(sw.cnt, 0) as stories_published_week,
    COALESCE(vs.today, 0) as visits_today,
    COALESCE(vs.week, 0) as visits_this_week,
    COALESCE(vs.prev_week, 0) as visits_last_week,
    COALESCE(vs.play_week, 0) as play_mode_visits_week,
    COALESCE(ss.likes, 0) as articles_liked,
    COALESCE(ss.dislikes, 0) as articles_disliked,
    COALESCE(ist.shares, 0) as share_clicks,
    COALESCE(ist.source_clicks, 0) as source_clicks,
    COALESCE(qs.responses, 0) as quiz_responses_count,
    COALESCE(ins.week, 0) as installs_this_week,
    COALESCE(ins.total, 0) as installs_total,
    COALESCE(subs.week, 0) as registrants_this_week,
    COALESCE(subs.total, 0) as registrants_total,
    COALESCE(ea.avg_engaged, 0) as avg_stories_engaged,
    COALESCE(ea.avg_swipes, 0) as avg_carousel_swipes,
    COALESCE(ea.avg_final, 0) as avg_final_slides_seen
  FROM user_topics ut
  ...
  LEFT JOIN subscriber_stats subs ON subs.topic_id = ut.id
