...
STYLE & COMPOSITION:
Clean line drawing - BOLD OUTLINES ONLY. Think Tintin (Hergé ligne claire style): confident black outlines define all shapes, minimal to zero interior detail, shadows as solid black fills (not hatched). Pure black (#000000) on white (#FFFFFF). Line weight variation for emphasis (thick/thin outlines), but NO crosshatching, NO stippling, NO texture strokes inside shapes. Composition through outline clarity and solid black shapes only.
...
Line work: Clean, bold outlines. Vary line thickness for visual hierarchy (thick=foreground/emphasis, thin=background/details). NO decorative strokes. NO texture. NO shading lines. Think comic book inking pass only - just the outlines that define shapes.
